package com.bijinsuo.web.controller.system;
...
import com.bijinsuo.common.core.domain.Ztree;
import com.bijinsuo.common.core.domain.entity.SysDept;
...
import com.bijinsuo.framework.shiro.service.SysPasswordService;
import com.bijinsuo.framework.shiro.util.AuthorizationUtils;
import com.bijinsuo.system.service.ISysDeptService;
import com.bijinsuo.system.service.ISysPostService;
import com.bijinsuo.system.service.ISysRoleService;
...
public class SysUserController extends BaseController {
    ...
    @RequiresPermissions("system:user:resetPwd")
    @Log(title = "重置谷歌密钥", businessType = BusinessType.UPDATE)
    @PostMapping("/resetGoogleSecretKey")
    @ResponseBody
    public AjaxResult resetGoogleSecretKeySave(Long userId) {
        final SysUser user = new SysUser();
        user.setUserId(userId);
        userService.checkUserAllowed(user);
        userService.checkUserDataScope(userId);
        if (userService.clearGoogleSecretKey(userId)) {
            return success();
        }
        return error();
    }
